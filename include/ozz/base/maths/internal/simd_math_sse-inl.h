...
#define OZZ_SSE_SELECT_F(_b, _true, _false)\
  _mm_xor_ps(_false, _mm_and_ps(_mm_castsi128_ps(_b),\
                                _mm_xor_ps(_true, _false)))\
...
OZZ_INLINE SimdFloat4 Normalize2(_SimdFloat4 _v) {
  ...
  const __m128 inv_len = _mm_div_ss(
    simd_float4::one(), _mm_sqrt_ss(sq_len));
...
OZZ_INLINE SimdFloat4 NormalizeSafe4(
  _SimdFloat4 _v, _SimdFloat4 _safe) {
  __m128 sq_len;
  OZZ_SSE_DOT4_F(_v, _v, sq_len);
  const __m128 inv_len = _mm_div_ss(simd_float4::one(), _mm_sqrt_ss(sq_len));
  const __m128 inv_lenxxxx = OZZ_SSE_SPLAT_F(inv_len, 0);
...
OZZ_INLINE SimdFloat4 Clamp(
  _SimdFloat4 _a, _SimdFloat4 _v, _SimdFloat4 _b) {
...
OZZ_INLINE SimdFloat4 Cos(_SimdFloat4 _v) {
  return _mm_set_ps(std::cos(GetW(_v)),
                    std::cos(GetZ(_v)),
                    std::cos(GetY(_v)),
                    std::cos(GetX(_v)));
}
...
OZZ_INLINE SimdFloat4 ACos(_SimdFloat4 _v) {
  return _mm_set_ps(std::acos(GetW(_v)),
                    std::acos(GetZ(_v)),
                    std::acos(GetY(_v)),
                    std::acos(GetX(_v)));
}
...
OZZ_INLINE SimdFloat4 Sin(_SimdFloat4 _v) {
  return _mm_set_ps(std::sin(GetW(_v)),
                    std::sin(GetZ(_v)),
                    std::sin(GetY(_v)),
                    std::sin(GetX(_v)));
}
...
OZZ_INLINE SimdFloat4 ASin(_SimdFloat4 _v) {
  return _mm_set_ps(std::asin(GetW(_v)),
                    std::asin(GetZ(_v)),
                    std::asin(GetY(_v)),
                    std::asin(GetX(_v)));
}
...
OZZ_INLINE SimdFloat4 Tan(_SimdFloat4 _v) {
  return _mm_set_ps(std::tan(GetW(_v)),
                    std::tan(GetZ(_v)),
                    std::tan(GetY(_v)),
                    std::tan(GetX(_v)));
}
...
OZZ_INLINE SimdFloat4 ATan(_SimdFloat4 _v) {
  return _mm_set_ps(std::atan(GetW(_v)),
                    std::atan(GetZ(_v)),
                    std::atan(GetY(_v)),
                    std::atan(GetX(_v)));
}
...
OZZ_INLINE SimdInt4 HAdd2(_SimdInt4 _v) {
  ...
  return _mm_castps_si128(_mm_move_ss(_mm_castsi128_ps(_v),
                                      _mm_castsi128_ps(hadd)));
}
...
OZZ_INLINE SimdInt4 Select(
  _SimdInt4 _b, _SimdInt4 _true, _SimdInt4 _false) {
...
OZZ_INLINE SimdInt4 CmpLe(_SimdInt4 _a, _SimdInt4 _b) {
  return _mm_andnot_si128(_mm_cmpgt_epi32(_a, _b),
                          _mm_cmpeq_epi32(_mm_setzero_si128(),
                                          _mm_setzero_si128()));
}
...
OZZ_INLINE SimdInt4 CmpGe(_SimdInt4 _a, _SimdInt4 _b) {
  return _mm_andnot_si128(_mm_cmplt_epi32(_a, _b),
                          _mm_cmpeq_epi32(_mm_setzero_si128(),
                                          _mm_setzero_si128()));
}
...
OZZ_INLINE Float4x4 Transpose(const Float4x4& _m) {
  ...
  const Float4x4 ret = {{_mm_unpacklo_ps(tmp0, tmp1),
                         _mm_unpackhi_ps(tmp0, tmp1),
                         _mm_unpacklo_ps(tmp2, tmp3),
                         _mm_unpackhi_ps(tmp2, tmp3)}};
  return ret;
}
...
OZZ_INLINE Float4x4 Invert(const Float4x4& _m) {
  ...
  const Float4x4 ret = {{_mm_mul_ps(det, minor0),
...
OZZ_INLINE SimdInt4 IsNormalized(const Float4x4& _m) {
  ...
  const __m128 dot = _mm_add_ps(
    _mm_add_ps(_mm_mul_ps(row0, row0), _mm_mul_ps(row1, row1)),
...
OZZ_INLINE SimdInt4 IsOrthogonal(const Float4x4& _m) {
  ...
  const SimdFloat4 cross =
    NormalizeSafe3(Cross3(_m.cols[0], _m.cols[1]), zero);
...
OZZ_INLINE SimdFloat4 ToQuaternion(const Float4x4& _m) {
  ...
  const __m128 one = _mm_castsi128_ps(
    _mm_srli_epi32(_mm_slli_epi32(ffff, 25), 2));
  ...
  const __m128 diag_sum = _mm_add_ps(
    _mm_add_ps(xx_yy_zz_xx, yy_zz_xx_yy), zz_xx_yy_zz);
  const __m128 diag_diff = _mm_sub_ps(
    _mm_sub_ps(xx_yy_zz_xx, yy_zz_xx_yy), zz_xx_yy_zz);
  const __m128 radicand = _mm_add_ps(
    OZZ_SSE_SELECT_F(mask_000f, diag_sum, diag_diff), one);
  ...
  zy_xz_yx = OZZ_SSE_SELECT_F(mask_0f00,
                              OZZ_SSE_SPLAT_F(_m.cols[2], 0), zy_xz_yx);
  ...
  yz_zx_xy = OZZ_SSE_SELECT_F(mask_f000,
                              OZZ_SSE_SPLAT_F(_m.cols[2], 1), yz_zx_xy);
  ...
  res0 = _mm_mul_ps(
    OZZ_SSE_SELECT_F(mask_f000, radicand, res0), OZZ_SSE_SPLAT_F(scale, 0));
  res1 = _mm_mul_ps(
    OZZ_SSE_SELECT_F(mask_0f00, radicand, res1), OZZ_SSE_SPLAT_F(scale, 1));
  res2 = _mm_mul_ps(
    OZZ_SSE_SELECT_F(mask_00f0, radicand, res2), OZZ_SSE_SPLAT_F(scale, 2));
  __m128 res3 = _mm_mul_ps(
    OZZ_SSE_SELECT_F(mask_000f, radicand, diff), OZZ_SSE_SPLAT_F(scale, 3));
  ...
  const __m128i cond1 = _mm_castps_si128(
    _mm_and_ps(_mm_cmpgt_ps(zz, xx), _mm_cmpgt_ps(zz, yy)));
...
OZZ_INLINE Float4x4 Float4x4::FromEuler(_SimdFloat4 _v) {
  ...
  const Float4x4 ret = {{
    simd_float4::Load(cx * cy, sx * sz - cx * sycz, cx * sysz + sx * cz, 0.f),
    simd_float4::Load(sy, cy * cz, -cy * sz, 0.f),
    simd_float4::Load(-sx * cy, sx * sycz + cx * sz, -sx * sysz + cx * cz, 0.f),
    simd_float4::w_axis()}};
  return ret;
}
...
OZZ_INLINE Float4x4 Float4x4::FromAxisAngle(_SimdFloat4 _v) {
  ...
  const __m128 v0 = _mm_mul_ps(
    _mm_mul_ps(one_minus_cos, _mm_shuffle_ps(_v, _v, _MM_SHUFFLE(3, 0, 2, 1))),
    _mm_shuffle_ps(_v, _v, _MM_SHUFFLE(3, 1, 0, 2)));
  const __m128 r0 = _mm_add_ps(_mm_mul_ps(_mm_mul_ps(one_minus_cos, _v), _v),
                               cos);
...
OZZ_INLINE ozz::math::Float4x4 operator+(
  const ozz::math::Float4x4& _a, const ozz::math::Float4x4& _b) {
  const ozz::math::Float4x4 ret = {{_mm_add_ps(_a.cols[0], _b.cols[0]),
                                    _mm_add_ps(_a.cols[1], _b.cols[1]),
                                    _mm_add_ps(_a.cols[2], _b.cols[2]),
                                    _mm_add_ps(_a.cols[3], _b.cols[3])}};
  return ret;
}

OZZ_INLINE ozz::math::Float4x4 operator-(
  const ozz::math::Float4x4& _a, const ozz::math::Float4x4& _b) {
  const ozz::math::Float4x4 ret = {{_mm_sub_ps(_a.cols[0], _b.cols[0]),
                                    _mm_sub_ps(_a.cols[1], _b.cols[1]),
                                    _mm_sub_ps(_a.cols[2], _b.cols[2]),
